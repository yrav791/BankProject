...
public class TC002_addNewCustomer extends BaseClass{
	...
	public void testAddcustomer() throws InterruptedException {
		...
		Thread.sleep(3000);
		anc.csetName(randomStringName().toUpperCase());
		anc.cclickGender();
		anc.csetDob("15-20-1996");
		anc.csetAddress(randomStringAddress().toLowerCase());
		anc.csetCity(randomStringName().toUpperCase());
		anc.csetState(randomStringName().toUpperCase());
		anc.csetPin(randomNumber());
		anc.csetTelephone(randomNumber());
		anc.csetEmail(randomStringName()+"@gmail.com");
