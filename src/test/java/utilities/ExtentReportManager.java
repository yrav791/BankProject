...
public void onStart(ITestContext testContext) {
  ...
  */
  String timeStamp=new SimpleDateFormat("yyyy.MM.dd.HH.mm.ss").format(new Date());// time st repName = "Test-Report-" + timeStamp + ".html";
   ...
   sparkReporter.config().setDocumentTitle("GuruBank Automation Report");
   sparkReporter.config().setReportName("GuruBank Functional Testing");
   sparkReporter.config().setTheme (Theme.DARK);

   extent=new ExtentReports();
   extent.attachReporter (sparkReporter);
   extent.setSystemInfo("Application", "GuruBank"); 
